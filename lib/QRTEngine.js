<script>
    ...
    var QRTE = {
        ...
        Init: function(paramObj) {
            ...
            if(typeof QRTESLIDERINITED === 'undefined') {
                //init the slider, if necessary
                QRTE.sliderInit();
            }
            //init the library
            QRTE.init();
            ...
            QRTE.setTrialData('InitPre[OnsetTime]', (typeof QRTEPREVIOUSSTORAGE != 'undefined')? QRTEPREVIOUSSTORAGE : undefined);
        ...
        Exit: function() {
            ...
            Qualtrics.SurveyEngine.addOnload(function() {
                ...
                onShowFn = function() {
                    ...
                    QRTEPREVIOUSSTORAGE = window.requestAnimationFrame.now();
                    QRTE.saveBlockData();
        ...
        startTrial: function(startId) {
            ...
            if(!QRTE.loadWaitingList.every()) {
                QRTE.debug('Process ' + startId + 'finished, waiting for other processes to finish')
                return;
            }
        ...
        */
        ...
        sliderLoadContainer: null,
        currentForm: null,
        previousForm: null,
        handlingSubmit: false,
        showResponse: false,
        initCount: 0,
        betweenSubmits: false,
        onSubmitStack: [],
        onSubmitFinish: [],
        nextTrialText: '',
    
        //Initialize the slider
        //There is a number of ghost variables that are initialised by this piece of code
        //There are global variables that are not pre-defined, and thus won't be overwritten
        // if this code is loaded for a second time.
        // These are: QRTESLIDERINITED, QRTECURRENTFORM, QRTEPREVIOUSFORM, QRTESLIDERLOADCONTAINER
        sliderInit: function() {
            QRTESLIDERLOADCONTAINER = new Element('div',{id: 'SliderLoadContainer'});
            QRTECURRENTFORM = $$('form')[0];
            ...
            Qualtrics.SurveyEngine.addOnload(function() {
                QRTECURRENTFORM.fire('QRTE:load');
            });
            ...
            Qualtrics.SurveyEngine.addOnload = function(f) {
                ...
                currentCount = QRTE.count(Qualtrics.SurveyEngine.QuestionInfo);
                ...
                try {
                    ...
                    QRTECURRENTFORM.observe('QRTE:load', obj.onload.bind(obj));
                    //Event.observe(QRTECURRENTFORM, 'SSload', obj.onload.bind(obj));
                }
                catch (e) {
                    console.error('SE API Error: ' + e);
                }
            }
            QRTE.submitPage = QRTE.handleSubmit;
            QRTESLIDERINITED = true;
        },
    
        handleSubmit: function() {
            if (QRTE.handlingSubmit === false) {
                console.log('Submitting');
                QRTE.handlingSubmit = true
    
                //Loop over submit stack
                for (index = 0; index < QRTE.onSubmitStack.length; index += 1) {
                    QRTE.onSubmitStack[index]();
                }
                QRTE.onSubmitStack = []
                QRTECURRENTFORM.request({
                    onComplete: QRTE.handleSubmitCallback
                });
    
                QRTE.betweenSubmits = true;
            }
        },
        ...
        loadNextPage: function() {
            ...
            QRTE.handlingSubmit = false;
            QRTEPREVIOUSFORM = QRTECURRENTFORM;
            Qualtrics.SurveyEngine.QuestionInfo = {}
            QRTE.sliderLoadContainer = jQuery(QRTESLIDERLOADCONTAINER).html(QRTE.nextTrialText)[0];
            QRTECURRENTFORM = jQuery(QRTESLIDERLOADCONTAINER).find('form')[0];
            jQuery(QRTEPREVIOUSFORM).replaceWith(jQuery(QRTECURRENTFORM));
            
            jQuery(QRTECURRENTFORM).show();
            
            if (QRTECURRENTFORM.select('.END_OF_SURVEY').length === 0) {
                QRTECURRENTFORM.fire('QRTE:load');
                QRTE.submitPage = QRTE.handleSubmit;
            }
            else {
                QRTE.submitPage = QRTECURRENTFORM.submit;
            }
            
        },
    
        handleSubmitCallback: function(e) {
            console.log('Stored response');
            QRTE.nextTrialText = e.responseText;
        },
    ...
    if(typeof QRTESS == 'undefined') {
        //QRTE SurveySlider controller layer
        QRTESS = {
            //This is where all SurveySlider logic should go.. eventually.
        };
    }
...
<style type="text/css">
    ...
    #SkinContent > #Buttons,.SkinInner > #Header {
        padding: 0 !important;
    }
</style>
