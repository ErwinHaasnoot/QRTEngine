<script>
    ...
    var QRTE = {
        ...
        initLib: function(blockId, exitItemTag) {
            ...
            blockDataString = Qualtrics.SurveyEngine.getEmbeddedData(this.blockDataField)|| '';
            columns = Qualtrics.SurveyEngine.getEmbeddedData(this.columnsField)|| '';
            idData = Qualtrics.SurveyEngine.getEmbeddedData(this.idField)|| '';
            exitQuestions = Qualtrics.SurveyEngine.getEmbeddedData(this.exitQuestions)|| '';
        ...
        Init: function(paramObj) {
            ...
            QRTE.initLib(paramObj.blockId, paramObj.exitItemTag);
            ...
            if (QRTE.waitForImageLoad === true) {
                ...
                for (; i < l; i += 1) {
                    images[i].observe('load', function(e) {
                        QRTE.startTrial(e.target.QRTELoadIndex);
                    })
    ...
    if (typeof QRTEController == 'undefined') {
        ...
        QRTEController = {
            ...
            init: function() {
                ...
                Qualtrics.SurveyEngine.addOnload = function(e) {
                        ...
                        var t = new Qualtrics.SurveyEngine.QuestionData;
                        this.Page.once("preready", function() {
                            try {
                                e.apply(t)
                            } catch (n) {
                                s.error("SE API Error: ", n, e)
                            }
                        })
                    }
                    //Set the Qualtrics setEmbeddedData to inside the QRTEController
                
                // Overwrite Page.__advancePage
                advancePage = Page.__advancePage
                
                Page.__advancePage = function(e,t,n) {
                    console.log(e,t,n)
                    advancePage.apply(Page,[e,t,n])
                }
            ...
            requestNextTrial: function(isLastTrial) {
                ...
                if (this.handlingSubmit === false && isLastTrial === false) {
                    ...
                    Page.adapter.next(Page.serializePageRuntime()).then(function(data) {QRTEController.requestNextTrialCallback(data)})
            ...
            loadNextTrial: function() {
                ...
                /** Load template into Qualtrics Page
                */
                
                    //Parse the HTML that was returned earlier and set it as the current trial container's contents
                Page._renderPageTemplate(this.nextTrialText,"next"); //this.nextTrialContainer = jQuery(this.nextTrialContainer).html(this.nextTrialText)[0];
                //Filter out the 'form' field, and append it back.
                
                this.currentForm = jQuery('form')[0];
            ...
            requestNextTrialCallback: function(e) {
                QRTEController.nextTrialText = e;
        ...
        if(document.readyState === "complete" && QRTEController.inited === false) {
          //Already loaded!
          QRTEController.init();
        }
        else {
