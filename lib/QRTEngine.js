<script>
    ...
    var QRTE = {
        ...
        Init: function(paramObj) {
            ...
            initIndex = paramObj.initQuestionIndex - 1 || 0;
            qArray = $('Questions').getElementsByClassName('QuestionOuter');
            for (var i = initIndex + 1; i < qArray.length; i += 1) {
        ...
        showStimulusQuestion: function(question) {
            ...
            if (question.conditional() === true) {
                ...
                if (question.loadConfig === true) {
                    //Load the Configuration of the Question
                ...
                if (question.onKeyPress !== undefined || question.questionElement.allowable !== '' || question.questionElement.cresp !== '') {
                    QRTE.addKeyListener(function(e) {
                        ...
                        //Store the Time of the Key Press and calculate the RT

                        if (QRTE.getTrialData(question.questionElement.QRTLibId + '[RTTime]') === undefined || QRTE.getTrialData(question.questionElement.QRTLibId + '[RTTime]') === '') {
                        ...
                        if (QRTE.isCorrectResponse(question.questionElement, RESP)) {
                            if (QRTE.getTrialData(question.questionElement.QRTLibId + '[ACC]') === undefined || QRTE.getTrialData(question.questionElement.QRTLibId + '[ACC]') === '') {
                                //console.log('correct');
                                QRTE.saveAcc(question.questionElement, 1);
                            }
                            if (question.onCorrectKey !== undefined) {
                                question.onCorrectKey.apply(question.questionElement, [e, RESP]);
                            }
                        }
                        else if (QRTE.isAllowableResponse(question.questionElement, RESP)) {
                            if (QRTE.getTrialData(question.questionElement.QRTLibId + '[ACC]') === undefined || QRTE.getTrialData(question.questionElement.QRTLibId + '[ACC]') === '') {
                                //console.log('correct');
                                QRTE.saveAcc(question.questionElement, 0);
                            }
                            if (question.onIncorrectKey !== undefined) {
        ...
        displayElement: function(paramObj) {
            ...
            else if (paramObj.delay > QRTE.refreshRate) {
                paramObj.el.style.display = 'none';
            }
